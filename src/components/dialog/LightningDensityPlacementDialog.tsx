...
interface IProps<T extends TLightningDensityGuiSchema> {
  isOpen: boolean;
  onClose: () => void;
  systemObj: Partial<T>;
}

export const LightningDensityPlacementDialog = <T extends TLightningDensityGuiSchema>({
  isOpen,
  onClose,
  systemObj,
}: IProps<T>) => {
  ...
  // 조명 밀도 이름
  const lightningDensityName = systemObj.name;
  ...
  const handleApply = () => {
        ...
        const nextZones = floor.zones.map((zone) => {
          ...
          if (sel) {
            return {
              ...zone,
              density: [systemObj],
              light_density: systemObj.density,
              lightning: [],
            };
          }
    ...
    setBeforeShapeInfo(updatedBefore as TFloorGuiSchema[]);
    setAfterShapeInfo(updatedAfter as TFloorGuiSchema[]);

    toast.success(`${lightningDensityName} 배치가 완료되었습니다.`);
    onClose();
  };
      ...
      >
        ...
        <DialogHeader className="sticky top-0 z-10 bg-white">
          ...
          <DialogDescription className={cn(isChecked ? "w-full" : "w-[344px]")}>
            {getPlacementDescription("single", lightningDensityName)}
          </DialogDescription>
        </DialogHeader>
