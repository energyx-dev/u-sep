...
export const LightningPlacementDialog = ({ isOpen, onClose, renewableSystemId }: IProps) => {
  ...
  const lightningName = useMemo(() => {
    return masterLightnings.find((v) => v.id === renewableSystemId)?.name;
  }, [masterLightnings, renewableSystemId]);
  ...
  const handleApply = () => {
    ...
    const addOrAccumulate = (lightningList: TLightningGuiSchema[], countToAdd: number) => {
      const list = lightningList ? [...lightningList] : [];
      const idx = list.findIndex((l) => l.id === renewableSystemId);

      if (idx !== -1) {
        return list.map((l, i) => (i === idx ? { ...l, count: (l.count || 0) + countToAdd } : l));
      }

      const newLightning = masterLightnings.find((v) => v.id === renewableSystemId);
      if (!newLightning) return list; // 안전 가드

      return [...list, { ...newLightning, count: countToAdd }];
    };

    const applySelections = (
      floors: typeof beforeFloors,
      selections: { count: number; floorId: string; name: string; zoneId: string }[],
    ) => {
      ...
      return floors.map((floor) => {
        ...
        const nextZones = floor.zones.map((zone) => {
          ...
          const nextLightning = addOrAccumulate(zone.lightning ?? [], sel.count);
          return {
            ...zone,
            density: [],
            light_density: 0,
            lightning: nextLightning,
          };
        });
    ...
    toast.success(`${lightningName} 배치가 완료되었습니다.`);
    onClose();
  };
      ...
      >
        ...
        <DialogHeader className="sticky top-0 z-10 bg-white">
          ...
          <DialogDescription className={cn(isChecked ? "w-full" : "w-[344px]")}>
            {getPlacementDescription("multiple", lightningName)}
          </DialogDescription>
        </DialogHeader>
        ...
        {/* 푸터 영역 */}
        <DialogFooter className="flex flex-col gap-2 bg-white">
