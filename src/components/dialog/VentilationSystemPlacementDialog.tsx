...
export const VentilationSystemPlacementDialog = <T extends TVentilationEngineAndGuiSchema>({
  isOpen,
  onClose,
  systemObj,
}: IProps<T>) => {
  ...
  // 환기 설비 이름
  const ventilationSystemName = systemObj.name;
  ...
  const handleApply = () => {
      ...
      return floors.map((floor) => {
        ...
        const nextZones = floor.zones.map((zone) => {
          ...
          if (sel) {
            return {
              ...zone,
              ventilation_system_id: systemObj.id,
            };
          }
    ...
    toast.success(`${ventilationSystemName} 배치가 완료되었습니다.`);
    onClose();
  };
      ...
      >
        ...
        <DialogHeader className="sticky top-0 z-10 bg-white">
          ...
          <DialogDescription className={cn(isChecked ? "w-full" : "w-[344px]")}>
            {getPlacementDescription("single", ventilationSystemName)}
          </DialogDescription>
        </DialogHeader>
